...
| **P2-DIRECT-FINAL-02 / P2-TRUTH-LOCK-01** | Container type decoupled from track type. `DetectedMediaKind` UNKNOWN & MIME `null` for generic containers without track proof. | `MP4_ISO_BMFF` mapped to `DetectedMediaKind.UNKNOWN` / MIME `null` | **PASS** |
