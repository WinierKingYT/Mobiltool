...
- **P2: Media / Video Downloader** - LOCKED / PASS
  - Direct HTTP/HTTPS: SUPPORTED / QUALIFIED (URL Security -> DNS Binding -> Safe Redirects -> Stream -> .part Staging -> Content Validation -> SHA-256 -> `StandardCopyOption.ATOMIC_MOVE` -> Canonical File)
...
- **P3: Library & Playback** - LOCKED / AWAITING USER ACTIVATION
...
* **Media Unit Tests**: PASS - LOCAL EXECUTION EVIDENCE (`.\gradlew.bat :media-extractor-api:test` - 65 offline tests)
